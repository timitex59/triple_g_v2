...
# -*- coding: utf-8 -*-
"""
signal_v6_28pairs.py
=====================
Signal detector v6 -- 28 paires LIVE via WebSocket TradingView
Parametres optimaux issus du walk-forward backtest v6 (70/30).

Usage :
  python signal_v6_28pairs.py                    # scan complet 28 paires
  python signal_v6_28pairs.py --pair EURJPY       # une paire
  python signal_v6_28pairs.py --loop              # boucle toutes les heures
  python signal_v6_28pairs.py --loop --interval 1800
  python signal_v6_28pairs.py --filter ROBUST     # seulement ROBUST/MOYEN/FRAGILE
"""
...
# (adx_thresh, sess_start, sess_end, use_holiday, af0, af, max_af, robustness)
# Source : backtest_tv_v6.py walk-forward 70/30, 3360 combinaisons
# ================================================================
PAIRS_CONFIG = {
    # ---- ROBUST (>= 60%) ----
    ...
    # ---- FRAGILE (<35%) -- parametres optimaux train ----
...
# ================================================================
# BUILD DF COMPLET (logique v6)
# ================================================================
def build_df(pair, h1, d1, w1, af0, af, maf):
    if h1 is None or d1 is None or w1 is None: return None
    ...
    d1['ema_bull'] = np.logical_and.reduce([d1[f'e{EMA_LENS[i]}'] > d1[f'e{EMA_LENS[i+1]}'] for i in range(7)])
    d1['ema_bear'] = np.logical_and.reduce([d1[f'e{EMA_LENS[i]}'] < d1[f'e{EMA_LENS[i+1]}'] for i in range(7)])

    w1['w_above'] = np.logical_and.reduce([w1['Close'] > w1[f'e{n}'] for n in EMA_LENS])
    w1['w_below'] = np.logical_and.reduce([w1['Close'] < w1[f'e{n}'] for n in EMA_LENS])
    w1['w_close'] = w1['Close']
    ...
    # SAR D/W (FIX v6 : inclus dans bias)
    dsar = psar(d1['High'], d1['Low'], af0, af, maf).rename('d_sar').shift(1)
    wsar = psar(w1['High'], w1['Low'], af0, af, maf).rename('w_sar').shift(1)
    ...
    df['sar']      = psar(h1['High'], h1['Low'], af0, af, maf)
...
# ================================================================
# AFFICHAGE RESUME TABULAIRE (compact pour 28 paires)
# ================================================================
...
def print_summary(results):
    ...
    print(f"\n{'='*90}")
    print(f"  SIGNAL v6 LIVE -- 28 paires -- {now} Paris")
    print(f"{'='*90}")

    # Trier : ROBUST > MOYEN > FRAGILE, puis par biais actif, puis alpha
    ...
    hdr = f"  {'Paire':<8} {'Rob':>4}% {'Label':<8} {'Biais':<8} {'Filtre':<8} {'Close':>10} {'SAR 1H':>10} {'Ref':>10} {'Pos':>6} {'WR%':>6} {'Signal / A surveiller'}"
    sep = "  " + "-"*88
    print(hdr); print(sep)
...
def print_active_signals(results):
    """Bloc detaille pour les paires avec signal ou position ouverte."""
    ...
    for r in actives:
        pair = r['pair']
        b = BIAIS_SYM.get(r['biais'])
        print(f"\n  {pair} [{r['label']} {r['rob']}%]  Biais:{b}  Close:{fp(r['close'],pair)}  SAR:{fp(r['sar'],pair)}")
        c = r['close']
        for p in r['longs']:
            ts  = r['idx'][p[2]]
...
def telegram_text(results):
    ...
    # Titre : TRAILING si trailing actif, ENTREE si nouveau signal, sinon rien
    if seen_trail:
...
# EXPORT CSV
# ================================================================
def export_csv(results, fname='signal_v6_28pairs_current.csv'):
    rows = []
    for r in results:
        if r.get('error'):
...
def main():
    parser = argparse.ArgumentParser(description="Signal v6 LIVE -- 28 paires TradingView")
