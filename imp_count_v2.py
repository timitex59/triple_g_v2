...
def update_currency_trend_state(
    ...
    Calcule aussi 2 billes 🟢/🔴 supplémentaires (cf. `_trend_arrow`),
    utilisées par `currency_trend_lines`, toutes deux absentes sur le tout
    premier run d'une chaîne (rien à comparer) :
    - `anchor_up_arrow`/`anchor_down_arrow` compare ce run à la référence de
      la chaîne (`anchor_up_pct`/`anchor_down_pct`, figée au %monté/%baissé
      du tout premier run de la chaîne -- vue d'ensemble depuis ce point de
      départ) ;
    - `up_arrow`/`down_arrow` compare ce run au run précédent immédiat
      (`prev_up_pct`/`prev_down_pct`, mis à jour à chaque run -- progression
      run à run, capte un retournement même si le niveau reste au-dessus de
      l'ancre).

    La chaîne démarre au tout premier run du jour (~00h Paris) et repart de
    zéro au premier run ≥ `CURRENCY_TREND_REFERENCE_RESET_HOUR_PARIS`h (14h)
    -- `period` ("am"/"pm") mémorise laquelle est active pour détecter cette
    transition."""
...
def currency_trend_lines(pairs: list[str], trend_state: dict) -> list[str]:
    ...
    Chaque ligne se termine par 2 billes 🟢/🔴 calculées par
    `update_currency_trend_state` : la référence de la chaîne (vs ~00h/14h
    Paris) puis la progression run à run (vs le run précédent) -- absentes
    (chacune indépendamment) quand la comparaison correspondante n'a rien à
    montrer, et toutes les deux sur le tout premier run d'une chaîne."""
