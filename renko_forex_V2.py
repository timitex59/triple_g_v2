...
"""
renko_forex_V2.py
Renko ATR(14) alignment M / W / D for selected OANDA Forex pairs.
...
STATE_PATH    = os.path.join(SCRIPT_DIR, "renko_forex_v2_state.json")
TRACKER_PATH  = os.path.join(SCRIPT_DIR, "renko_forex_v2_tracker.json")
HISTORY_PATH  = os.path.join(SCRIPT_DIR, "renko_forex_v2_pnl_history.json")
PORTFOLIO_PATH = os.path.join(SCRIPT_DIR, "renko_forex_v2_portfolio.json")
SPREAD_CACHE_PATH = os.path.join(SCRIPT_DIR, "renko_forex_v2_spreads.json")
...
def scan_symbol(symbol: str, name: str, atr_length: int,
    ...
    # Timeframes: M (monthly) / W (weekly) / D (daily)
    brick_mn = _fetch_renko_with_fallback(symbol, "M", atr_length, 200, debug)
    if not brick_mn:
        if debug:
...
def init_bias_from_history(symbol: str, start_date_utc: int,
    ...
    # Timeframes: M / W / D
    bars_3m = fetch_tv_ohlc(symbol, "M", 200)
...
def is_trade_candidate(s: ForexSnapshot) -> bool:
    direction = trade_direction(s)
    if direction == 0 or s.pnl_pct is None or s.pnl_pct <= 0:
        return False
    if s.streak_w < 1:
        return False
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Renko M/W/D alignment — OANDA Forex")
...
def main() -> int:
    ...
    if not args.filter_positive:
        for s in snaps:
            ...
            print(f"{s.name:<6} | close={s.close:>10.2f} | M={s.px_mn:+d}({s.streak_3m}) W={s.px_w1:+d}({s.streak_m}) D={s.px_d1:+d}({s.streak_w}) | "
                  f"Side {side_txt} | "
