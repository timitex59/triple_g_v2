...
class TestMultiLayerStrategy(unittest.TestCase):
    ...
    def test_evaluate_pair_aplus(self):
        # Mock fibo and vivier entries to yield Grade A+
        class MockFiboState:
            direction = 1
        ...
        score = evaluate_pair_multilayer("AUDUSD", self.composites, self.align_pairs, fibo_results, vivier_state)
        self.assertIsNotNone(score)
        self.assertEqual(score.pair, "AUDUSD")
        self.assertEqual(score.total_score, 100)
        self.assertEqual(score.grade, "A+")
        self.assertIn("FIOS", score.layers_passed)
    ...
    def test_format_section(self):
        scores = [
            MultiLayerScore(
                ...
                total_score=100,
                grade="A+",
                layers_passed=["FIOS", "Renko M/W/D", "Fibo 50%", "Vivier H1"],
                daily_chg=0.50,
                tag_mwd="M+ W+ D+",
            )
        ]
        lines = format_multilayer_section(scores)
        self.assertTrue(any("TRADES GRADE A+" in line for line in lines))
