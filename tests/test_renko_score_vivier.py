...
class VivierStateTests(unittest.TestCase):
    ...
    def test_missing_market_row_does_not_erase_tracking(self):
        state, _ = update_vivier([row("AUDJPY", 1, -1, 0)], {}, NOW)
    ...
    def test_groups_are_ranked_by_absolute_base_score(self):
        initial_rows = [
            row("BULL00", 1, -1, -1), row("BULL83", 1, -1, -1),
            row("BULL33", 1, -1, -1), row("BEAR00", -1, 1, 1),
            row("BEAR83", -1, 1, 1), row("BEAR33", -1, 1, 1),
        ]
        state, _ = update_vivier(initial_rows, {}, NOW)
        current_rows = [
            row("BULL00", 1, -1, -1), row("BULL83", 1, 1, 0),
            row("BULL33", 1, 0, -1), row("BEAR00", -1, 1, 1),
        ...
        self.assertEqual([pair for pair, _ in bull], ["BULL83", "BULL33", "BULL00"])
        self.assertEqual([pair for pair, _ in bear], ["BEAR83", "BEAR33", "BEAR00"])
