...
"""
sarenki_V2_forex.py
Python mirror of sarenki_V1.pine — Momentum removed.
Scans hourly and sends Telegram alerts.

Signal conditions:
  TREND = BULL  → State + MTF both BULL
  TREND = BEAR  → State + MTF both BEAR
...
def scan_pair(pair, debug=False):
    ...
    # ── TREND : State + MTF ───────────────────────────────────────────────────
    trend = 1 if g_state == 1 and mtf == 1 else \
           -1 if g_state == -1 and mtf == -1 else 0
...
LOG_FILE = os.path.join(os.path.dirname(__file__), "sarenki_V2_forex_log.json")
...
def parse_args():
    p = argparse.ArgumentParser(description="Sarenki V2 Forex signal scanner")
