...
def fetch_data_tv(pair_yahoo, interval_code, n_candles=200, live_seconds=0):
    """
    ...
    try:
        ...
        while True:
            try:
                ...
                if now >= hard_deadline:
                    break
                if (now - start_t) >= 10 and not series_done:
                    break
                ...
                if not res:
                    continue
                if TV_DEBUG and raw_log_limit > 0:
                    print(f"[TV] raw {pair_yahoo} tf={interval_code}: {res[:400]}")
                    raw_log_limit -= 1
                buffer += res
                ...
                for payload in payloads:
                    ...
                    if message.get("m") == "series_completed":
                        series_done = True
                        if live_seconds and live_deadline is None:
                            live_deadline = time.time() + live_seconds
                        if TV_DEBUG:
                            pass
                    for bar in extract_tv_bars(message):
                        ...
                        if start != -1 and end != -1:
                            ...
                            for item in data:
                                if isinstance(item, dict) and "v" in item:
                                    bar = item["v"]
                                    if isinstance(bar, (list, tuple)) and len(bar) >= 5:
                                        bars_by_ts[bar[0]] = bar
                    except Exception:
                        pass
                if series_done and not live_seconds:
                    break
        ...
        if bars_by_ts:
            ...
            extracted_df = bars_to_dataframe(ordered)
            if TV_DEBUG:
                pass
...
def send_telegram_summary(results):
    ...
    for r in results:
        ...
        was_aligned_today = prev_aligned_today.get(pair)
        was_neutral = prev_neutral_list.get(pair)
    ...
    # Only show aligned pairs (two same-color icons).
    if changed_lines:
        if lines:
            lines.append("")
            lines.append("ALERTE")
