...
"""
renko_forex_V1.py
Renko ATR(14) alignment MN / W1 / D1 for selected OANDA Forex pairs.
...
STATE_PATH    = os.path.join(SCRIPT_DIR, "renko_forex_v1_state.json")
TRACKER_PATH  = os.path.join(SCRIPT_DIR, "renko_forex_v1_tracker.json")
HISTORY_PATH  = os.path.join(SCRIPT_DIR, "renko_forex_v1_pnl_history.json")
PORTFOLIO_PATH = os.path.join(SCRIPT_DIR, "renko_forex_v1_portfolio.json")
SPREAD_CACHE_PATH = os.path.join(SCRIPT_DIR, "renko_forex_v1_spreads.json")
...
def scan_symbol(symbol: str, name: str, atr_length: int,
    ...
    bars_d = fetch_tv_ohlc(symbol, "W", 16)
    if not bars_d:
        if debug:
...
def update_portfolio_simulation(
    ...
    for sym, pos in ordered_positions:
        ...
        roc_txt = f" {roc:+.2f}%" if roc is not None else ""
        side = pos.get("side", "LONG")
        side_icon = "🟢" if side == "LONG" else "🔴"
        summary.append(f"{side_icon}{side} {pos.get('name', sym)} ({qty}){roc_txt}{new_txt}")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Renko MN/W1/D1 alignment — OANDA Forex")
